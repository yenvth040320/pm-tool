  ...
  <div>
    <a-card class="mb-4">
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <a-input
                v-model="filters.content_check_jp"
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.use_flg')"
              prop="use_flg"
            >
              <a-input
                v-model="filters.use_flg"
                :placeholder="$t('module.functionTestCase.use_flg')"
              >
                <font-awesome-icon
                  slot="prefix"
                  icon="heading"
          ...
          >
            ...
            >
              <a-input
                v-model="filters.expect_result_jp"
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.function_id')"
              prop="function_id"
            >
              <a-select
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.mst_validation_id')"
              prop="mst_validation_id"
            >
              <a-select
                v-model="filters.mst_validation_id"
                :placeholder="$t('module.mstValidation')"
              >
                <a-select-option
                  v-for="(item, index) in mstValidations"
                  :key="index"
                  :value="item.id"
                >
                  {{ item.name }}
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.order')"
              prop="order"
            >
              <a-input
                v-model="filters.order"
                :placeholder="$t('module.functionTestCase.order')"
              >
                <font-awesome-icon
                  slot="prefix"
                  icon="heading"
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.project_id')"
              prop="project_id"
            >
              <a-select
          ...
          >
            <a-form-model-item
              :label="$t('module.functionTestCase.version_code')"
              prop="version_code"
            >
              <a-input
                v-model="filters.version_code"
                :placeholder="$t('module.functionTestCase.version_code')"
              >
                <font-awesome-icon
                  slot="prefix"
                  icon="heading"
      ...
      >
        ...
        <template
          slot="mst_validation_id"
          slot-scope="text, record"
        >
          <div>
            {{ record.mstValidation.name }}
          </div>
        </template>
        <template
          slot="project_id"
    ...
    >
      <template slot="title">
        ...
        {{ currentId ? $t('common.edit') : $t('common.create') }} {{ $t('module.functionTestCase') }}
      </template>

      <a-spin :spinning="loading">
        <functionTestCase-form
          :id="currentId"
...
import FunctionTestCaseForm from '~/components/templates/functionTestCase/FunctionTestCaseForm'
...
export default {
  ...
  data() {
    return {
      ...
      currentId: 0,
      filters: {
        content_check: this.$route.query.content_check || '',
        content_check_jp: this.$route.query.content_check_jp || '',
        use_flg: this.$route.query.use_flg || '',
        event_pk: this.$route.query.event_pk || '',
        expect_result: this.$route.query.expect_result || '',
        expect_result_jp: this.$route.query.expect_result_jp || '',
        ...
        mst_validation_id: this.$route.query.mst_validation_id || '',
  ...
  computed: {
    header() {
      return [
        ...
        {
          title: this.$t('module.functionTestCase.content_check_jp'),
          dataIndex: 'content_check_jp',
          sorter: true
        },
        {
          title: this.$t('module.functionTestCase.use_flg'),
          dataIndex: 'use_flg',
          sorter: true
        },
        {
        ...
        {
          title: this.$t('module.functionTestCase.expect_result_jp'),
          dataIndex: 'expect_result_jp',
          sorter: true
        },
        {
          title: this.$t('module.functionTestCase.function_id'),
        ...
        {
          title: this.$t('module.functionTestCase.mst_validation_id'),
          dataIndex: 'mst_validation_id',
          scopedSlots: { customRender: 'mst_validation_id' },
          sorter: true
        },
        {
        ...
        {
          title: this.$t('module.functionTestCase.project_id'),
          dataIndex: 'project_id',
          scopedSlots: { customRender: 'project_id' },
          sorter: true
        },
        {
          title: this.$t('module.functionTestCase.version_code'),
  ...
  methods: {
    ...
    async getMstValidations() {
      const { data: { result: { data } } } = await this.$api.mstValidation.list()
