  ...
  <div>
    <a-card class="mb-4">
      ...
      <div class="action-function">
        <div class="pb-4">
          <a-button
            ...
            @click="checkedAll"
          >
            {{ allChecked ? $t('common.remove_all') : $t('common.check_all') }}
          </a-button>
          <a-button
            html-type="button"
            type="primary"
        ...
        <div class="pb-4">
          <a-button
            ...
            type="primary"
            class="min-w-100"
            :disabled="!checkedId.length || loading"
            @click="confirmLockAll"
          >
            {{ $t('common.lock_all') }}
          </a-button>
          <a-button
            ...
            class="min-w-100"
            :disabled="!checkedId.length || loading"
            @click="checkAllVersion"
          >
            {{ $t('common.increase_all') }}
          </a-button>
        </div>
      </div>
      ...
      >
        ...
        >
          <a-checkbox
            :checked="allChecked || checkedId.includes(record.id)"
            @change="(e) => onChangeCheckbox(record.id, e)"
          />
        </template>
        <template
          slot="name"
        ...
        <template
          ...
          slot-scope="text, record"
        >
          <a-tooltip placement="top" :title="$t('module.function.level_icon_tooltip')">
            <NuxtLink
              :to="`/function/${record.id}/version/${record.latest_version}#tab_logics`"
              class="icon-link"
            >
              <a-icon
                v-if="checkFunctionHasLevel(record.functionItems)"
                type="dollar"
              />
            </NuxtLink>
          </a-tooltip>
          <a-tooltip placement="top" :title="$t('module.function.logic_icon_tooltip')">
            <NuxtLink
              :to="`/function/${record.id}/version/${record.latest_version}#tab_logics`"
              class="icon-link"
            >
              <a-icon
                v-if="checkFunctionItemHasLogic(record.functionItems)"
                type="interaction"
              />
            </NuxtLink>
          </a-tooltip>
          <a-tooltip placement="top" :title="$t('module.function.no_item_tooltip')">
            <NuxtLink
              :to="`/function/${record.id}/version/${record.latest_version}#tab_items`"
              class="icon-link"
            >
              <a-icon
                v-if="record.functionItems.length === 0"
                type="ordered-list"
              />
            </NuxtLink>
          </a-tooltip>
          <a-tooltip placement="top" :title="$t('module.function.api_icon_tooltip')">
            <NuxtLink
              :to="`/function/${record.id}/version/${record.latest_version}#tab_datasources`"
              class="icon-link"
            >
              <a-icon
                v-if="checkFunctionItemHasDataSource(record.functionItems)"
                type="api"
              />
            </NuxtLink>
          </a-tooltip>
        ...
        >
          ...
          <a-button
            ...
            size="small"
            title="Tăng version"
            :disabled="loading || allVersionsLocked(record) || record.use_flg === USE_FLG_VALUE.no"
        ...
        >
          ...
          >
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.design_lock') }}
              </template>
              <a-button
                v-if="item.design_lock_flg === LOCK_FLG_VALUE.no"
                html-type="button"
                type="primary"
                title="Khoá version"
                size="small"
                :disabled="loading || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.yes, 1)"
              >
            ...
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.design_unlock') }}
              </template>
              <a-button
                v-if="item.design_lock_flg === LOCK_FLG_VALUE.yes"
                html-type="button"
                type="primary"
                size="small"
                title="Mở version"
                :disabled="loading || isVersionDisabled(item) || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.no, 1)"
              >
            ...
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.code_lock') }}
              </template>
              <a-button
                v-if="item.code_lock_flg === LOCK_FLG_VALUE.no"
                html-type="button"
                type="primary"
                title="Khoá version"
                size="small"
                :disabled="loading || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.yes, 2)"
              >
                <font-awesome-icon icon="lock" />
              </a-button>
            </a-tooltip>
            ...
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.code_unlock') }}
              </template>
              <a-button
                v-if="item.code_lock_flg === LOCK_FLG_VALUE.yes"
                html-type="button"
                type="primary"
                size="small"
                title="Mở version"
                :disabled="loading || isVersionDisabled(item) || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.no, 2)"
              >
                <font-awesome-icon :icon="['fas', 'lock-open']" />
              </a-button>
            </a-tooltip>
            ...
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.testcase_lock') }}
              </template>
              <a-button
                v-if="item.testcase_lock_flg === LOCK_FLG_VALUE.no"
                html-type="button"
                type="primary"
                title="Khoá version"
                size="small"
                :disabled="loading || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.yes, 3)"
              >
                <font-awesome-icon icon="lock" />
              </a-button>
            </a-tooltip>
            ...
            <a-tooltip>
              <template slot="title">
                {{ $t('module.versionFunction.testcase_unlock') }}
              </template>
              <a-button
                v-if="item.testcase_lock_flg === LOCK_FLG_VALUE.yes"
                html-type="button"
                type="primary"
                size="small"
                title="Mở version"
                :disabled="loading || isVersionDisabled(item) || item.function.use_flg === USE_FLG_VALUE.no"
                @click="changeVersion(item.id, LOCK_FLG_VALUE.no, 3)"
              >
                <font-awesome-icon :icon="['fas', 'lock-open']" />
              </a-button>
            </a-tooltip>
    ...
    <IncreaseAllVersion ref="increaseAllVersion" @save="save" />
  </div>
</template>
<script>
...
import IncreaseVersion from '~/components/templates/function/IncreaseVersion.vue'
import IncreaseAllVersion from '~/components/templates/function/IncreaseAllVersion.vue'
  ...
  computed: {
    header() {
      return [
        ...
        {
          title: this.$t('module.function.short_name'),
