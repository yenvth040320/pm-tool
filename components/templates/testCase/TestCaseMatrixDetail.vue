<template>
  <div>
    <a-spin :spinning="spin">
      <div v-if="matrix.id && index !== -1" class="heading-page">
        {{ $t('module.test_case_matrix.update') }}
        <div>
          <a-button
          ...
          >
            ...
            {{ $t('module.test_case_matrix.copy_code_for_item') }}
          </a-button>
        </div>
      </div>
      <a-row
        v-if="matrix.id && index !== -1"
        class="main-row pt-3"
      >
        <div
          class="flex-box"
        ...
        <div class="flex-box">
          ...
          >
            ■<Editable :text="matrix.name" @change="onChangeMatrixName(matrix, 'name', $event)" />
            <a-icon
            ...
            />
            <a-icon
              v-else-if="matrix.id && matrix.testcaseNumbers.length === 0"
              type="setting"
              class="icon-gen-matrix"
      ...
      <a-divider />
      <div>
        {{ $t('module.test_case_matrix.create') }}
      </div>
      <AddTestCaseMatrixComponent
        v-if="indexMatrix !== -1"
        :matrix="newMatrix"
        :index="indexMatrix"
      ...
      />
      <div>
        {{ $t('module.test_case_matrix.case_note') }}
        <p class="note">
...
import AddTestCaseMatrixComponent from '~/components/templates/testCase/AddTestCaseMatrixComponent'
...
export default {
  ...
  watch: {
    testcaseMatrixs: {
      immediate: true,
      handler(newVal) {
        this.pushEmptyCustomTestcase(newVal)
  ...
  methods: {
    pushEmptyCustomTestcase(newVal) {
      ...
      this.indexMatrix = this.dataTestcaseMatrix.findIndex(record => record.id === null || record.id === 0)
      this.newMatrix = this.dataTestcaseMatrix.find(record => record.id === null || record.id === 0)
    },
    /**
    ...
    onChangeGroupName(matrix, dataIndex, value) {
      ...
      this.matrix.testcaseGroup[dataIndex] = value
    },
    ...
    deleteInputMatrixRow(matrix, index, record) {
      console.log(matrix.testcaseNumbers, 'testcaseNumbers')
      matrix.testcaseMatrixInputs.splice(index, 1)
      const testcaseInputResult = record.testcase_input_results.filter(result => result.testcase_matrix_input_result === 1)

      testcaseInputResult.forEach(result => {
        const indexToRemove = matrix.testcaseNumbers.findIndex(item => item.testcase_number_pk === result.testcase_number_pk)

        if (indexToRemove !== -1) {
          matrix.testcaseNumbers.splice(indexToRemove, 1)
        }
      })
      if (record.id > 0) {
    ...
    deleteOutputMatrixRow(matrix, index, record) {
      ...
      const testcaseOutputResult = record.testcase_output_results.filter(result => result.testcase_matrix_output_result === 1)

      testcaseOutputResult.forEach(result => {
        const indexToRemove = matrix.testcaseNumbers.findIndex(item => item.testcase_number_pk === result.testcase_number_pk)
    ...
    getTestcaseId(testcaseNumbers, matrix) {
      ...
      testcaseNumbers.forEach((i, index) => {
        ...
        this.selectedRowInput.forEach(selectedInputId => {
          ...
          if (selectedInput) {
            const isResultExist = selectedInput.testcase_input_results.some(result => result.testcase_number_pk === i.testcase_number_pk && result.testcase_matrix_input_result === 1)
            isResultExistArray.push(isResultExist)
            if (isResultExist) {
        ...
        const customCellFunction = () => {
          const classes = {
            ...
            selected: isResultExistArray.includes(true)
          }
          return {
            class: classes
          }
              ...
              return (
                <div class="testcase-number">
                  ...
                  <div class="testcase-number-action-btn">
                    <a-icon type="plus" class="icon icon-plus" title="Thêm" onClick={() => this.addTestcaseNumber(testcaseNumbers, i.testcase_matrix_pk, index)}></a-icon>
                    <a-icon type="minus" class="icon icon-minus" title="Xoá" onClick={() => this.deleteTestcaseNumber(testcaseNumbers, index, i, matrix)}></a-icon>
                    <a-icon type="copy" class="icon icon-copy" title="Copy code test case" onClick={() => this.copyCodeTest(matrix, i)}></a-icon>
                  </div>
                </div>
              )
            } else {
    ...
    getMatrixOutputResultHeaders(matrix) {
      ...
      testcaseNumbers.forEach(i => {
        ...
        this.selectedRowOutput.forEach(selectedOutputId => {
          ...
          if (selectedOutput) {
            const isResultExist = selectedOutput.testcase_output_results.some(result => result.testcase_number_pk === i.testcase_number_pk && result.testcase_matrix_output_result === 1)
            isResultExistArray.push(isResultExist)
            if (isResultExist) {
    ...
    addTestcaseNumber(testcaseNumbers, pk, index) {
      ...
      const hasTemporaryPk = testcaseNumbers.some(record => record.temporary_pk !== undefined)

      const maxTemporaryPk = hasTemporaryPk
        ? Math.max(...testcaseNumbers.filter(record => record.temporary_pk !== undefined).map(record => record.temporary_pk))
        : 0

      const newTestcaseNumber = {
        id: 0,
        testcase_number_pk: hasTemporaryPk ? maxTemporaryPk + 1 : maxTestcaseNumberPk + 1,
        temporary_pk: hasTemporaryPk ? maxTemporaryPk + 1 : maxTestcaseNumberPk + 1,
        project_id: this.project_id,
        function_id: this.function_id,
        version_code: this.version_code,
    ...
    async copyCodeTest(matrix, item) {
      this.$emit('changeSpin', true)

      try {
        const dataTestcaseNumber = {
          testcaseNumber: item
        }
        const { data: { result: { data } } } = await this.$api.testcaseMatrix.copyCodeTestCase(dataTestcaseNumber)
        if (data !== null) {
          const testcaseMatrixInput = matrix.testcaseMatrixInputs.find(input => input.testcase_matrix_input_pk === item.testcase_matrix_input_pk)
          const testcaseMatrixOutput = matrix.testcaseMatrixOutputs.find(input => input.testcase_matrix_output_pk === item.testcase_matrix_output_pk)
          const indentedData = data.split('\n').map((line, index) => index === 0 ? line : `      ${line}`).join('\n')
          const testDescription = `

test.describe('${matrix.function.code}: ${matrix.function.name}/version: ${matrix.version_code}', () => {
  test.describe('${matrix.testcaseGroup.name}', () => {
    test.describe('[${matrix.name}]', () => {
      test('[${matrix.function.code}: ${matrix.function.name}/version: ${matrix.version_code}] ${matrix.functionItem !== null ? ` [ ${matrix.functionItem.item_num}: ${matrix.functionItem.name}` : ''}] - ${testcaseMatrixInput.test_object} - ${testcaseMatrixOutput.expected_result}', async ({ page }) => {
        ${indentedData}
      });
...
`
          await navigator.clipboard.writeText(testDescription)
          this.$message.success(this.$t('module.functionItem.copy_message'))
        } else {
    ...
    async copyAllCode() {
      ...
      try {
        if (this.matrix.id !== 0) {
          ...
          const { data: { result: { data } } } = await this.$api.testcaseMatrix.copyCodeTestCaseForItem(dataCopy)
          if (data !== '') {
