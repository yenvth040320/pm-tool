<template>
  <a-card class="mb-4 box-content">
    <template slot="title">
      <span class="title">
        {{ $t("module.requirement.estimate.joint_work") }}
      </span>
    </template>
    <div class="pb-4 container">
      <div>
        ...
        <span v-if="!isUserEstimate && startEstimate === true && !isHasPermision">{{ $t('module.requirement.text_message') }}</span>
      </div>
      <div>
    ...
    >
      ...
      >
        <template slot="name" slot-scope="text, record">
          <a-input v-if="record.mst_life_cycle_id && isUserEstimate && record.task && record.task.status === TASK_STATUS_VALUE.new && ((record.approve_status === APPROVE_STATUS_VALUE.not_approve) || isHasPermision)" v-model="record.title" class="title-task" />
...
import { MST_LIFECYCLE_TYPE_VALUE, TASK_TYPE_VALUE, APPROVE_STATUS_ESTIMATE, APPROVE_STATUS_VALUE, TASK_STATUS_VALUE, CKEDITOR_TOOLBAR_SETTING, API_ROOT_URL } from '~/constants'
...
export default {
  ...
  methods: {
    ...
    async gotoStart() {
      ...
      this.startEstimate = true
      await addDoc(collection(db, 'Process_estimate'), {
    ...
    */
    checkProcessEstimate() {
      const q = query(
        collection(db, 'Process_estimate'),
    ...
    */
    async cancel() {
      ...
      querySnapshot.forEach(data => {
        const docRef = doc(db, 'Process_estimate', data.id)
    ...
    async handleSubmit() {
      try {
        ...
        await this.$api.requirementEstimate.createOrUpdateEstimateCommon(params)
        this.cancel()
        this.$notification.success({
          message: this.$t('text.successfully')
        })
