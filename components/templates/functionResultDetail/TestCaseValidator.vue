    ...
    <a-card class="mb-4 no-border">
      ...
      >
        ...
        <template
          ...
          slot-scope="text, record"
        >
          <div v-if="record.functionTestCase.mstItemProperty">
            {{ record.functionTestCase.mstItemProperty?.name }}
          </div>
        </template>
  ...
  methods: {
    ...
    updateFunctionResultDetailData(newVal) {
      this.functionResultDetailData = newVal.filter(
        record =>
          record.functionTestCase.mst_item_property_id !== null ||
          record.functionTestCase.mst_testcase_validation_id !== null ||
          (record.functionTestCase.mst_testcase_common_id === null &&
            record.functionTestCase.event_pk === null &&
            record.functionTestCase.logic_pk === null)
      )
    },
