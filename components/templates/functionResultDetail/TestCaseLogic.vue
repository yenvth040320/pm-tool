  ...
  <div>
    ...
    <a-card class="mb-4 no-border">
      ...
      >
        ...
        >
          <div v-if="record.functionTestCase?.functionItem">
            {{ record.functionTestCase?.functionItem?.name }}
          </div>
        </template>
        <template
          slot="event_pk"
          slot-scope="text, record"
        >
          <div v-if="record.functionTestCase.event">
            {{ record.functionTestCase?.event.mstEvent?.name }}
          </div>
        </template>
        <template
          slot="logic_pk"
          slot-scope="text, record"
        >
          <div v-if="record.functionTestCase.logic">
            <span v-if="record.functionTestCase.logic.logic_type === LOGIC_TYPE_VALUE.sequence">
              {{ record.functionTestCase.logic.logic_detail }}
            </span>
            <span v-else-if="record.functionTestCase.logic.logic_type === LOGIC_TYPE_VALUE.goto">
              {{ $t('module.functionEventLogical.goto') }}: {{ record.functionTestCase.logic.functionGoto?.name }}
            </span>
            <span v-else-if="record.functionTestCase.logic.logic_type === LOGIC_TYPE_VALUE.if">
              {{ $t('module.functionEventLogical.if') }} ({{ record.functionTestCase.logic.conditions }})
            </span>
            <span v-else-if="record.functionTestCase.logic.logic_type === LOGIC_TYPE_VALUE.elseif">
              {{ $t('module.functionEventLogical.elseif') }} ({{ record.functionTestCase.logic.conditions }})
            </span>
            <span v-else>
...
import { LOCK_FLG_VALUE, USE_FLG_VALUE, RESULT_FLG, LOGIC_TYPE_VALUE } from '~/constants'
  ...
  methods: {
    ...
    updateFunctionResultDetailData(newVal) {
      this.functionResultDetailData = newVal.filter(
        record => record.functionTestCase.event_pk !== null && record.functionTestCase.logic_pk !== null
      )
    },
