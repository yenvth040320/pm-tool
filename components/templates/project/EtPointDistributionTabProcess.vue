        ...
        <a-card class="mb-4 no-border">
          ...
          >
            ...
            <a-table
              ...
              :data-source="model.version_function_life_cycles"
              :pagination="false"
...
export default {
  ...
  computed: {
    header() {
      return [
        ...
        {
          title: this.$t('module.tab_process.total_task_point'),
  ...
  methods: {
    open(data) {
      ...
      this.model.version_function_life_cycles = this.model.version_function_life_cycles.map(child => {
        if (child.mst_life_cycle_status_id === undefined) {
      ...
      const totalEtPercent = this.model.version_function_life_cycles.reduce((total, item) => {
        const pointEt = typeof item.mst_life_cycle.et_percent === 'number'
          ? item.mst_life_cycle.et_percent
          : 0
        return total + pointEt
      }, 0)
      const totalPointAdd = this.model.version_function_life_cycles.reduce((total, item) => {
        const pointEt = typeof item.point_et === 'number'
          ? item.point_et
          : 0
        return total + pointEt
      }, 0)
      this.model.version_function_life_cycles.push({
        key: 'total',
    ...
    calculateTotalPointEt(pointEts) {
      if (pointEts.length > 0) {
        const total = pointEts.reduce((total, pointObj) => {
          const pointEt = typeof pointObj.point_et === 'number'
            ? pointObj.point_et
            : 0
          return total + pointEt
        }, 0)
    ...
    getAutoPoint() {
      if (this.checkAuto) {
        this.model.version_function_life_cycles.forEach(child => {
     ...
     */
    calTotalEtPercent() {
      const newArr = [...this.model.version_function_life_cycles]
      newArr.pop()
      this.totalEtPercent = (newArr.reduce((total, item) => {
        const etPercent = typeof item.mst_life_cycle.et_percent === 'number'
          ? item.mst_life_cycle.et_percent
          : 0
    ...
    async handleSubmit() {
      ...
      try {
        let data = [...this.model.version_function_life_cycles]
        data.pop()
