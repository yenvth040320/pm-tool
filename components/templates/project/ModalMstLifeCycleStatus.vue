        ...
        <div class="p-4">
          ...
          >
            <a-row>
              ...
              >
                <a-form-model-item
                  :label="$t('module.tab_process.status_table')"
                  prop="mst_life_cycle_status_id"
                >
                  <a-radio-group v-model="model.version_function_life_cycles.find(item => item.mst_life_cycle_id === mstLifeCycleId).mst_life_cycle_status_id">
                    <a-radio v-for="(item, index) in mstLifeCycles.find(lifeCycle => lifeCycle.id === mstLifeCycleId).mstLifeCycleStatuses" :key="index" :style="radioStyle" :value="item.id">
                      {{ item.name }}
                    </a-radio>
                  </a-radio-group>
                </a-form-model-item>
